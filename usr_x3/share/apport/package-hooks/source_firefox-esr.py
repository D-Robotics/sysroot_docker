...
class Plugin(object):
    ...
    def dump(self):
        if self.path.startswith(os.path.join(os.environ['HOME'], '.mozilla', 'firefox-esr')):
            location = "[Profile]"
        else:
...
class Prefs:
    ...
    def __init__(self, profile_path, extra_paths=None, whitelist=None, blacklist=None):
        ...
        if profile_path != None:
            locations = [
                "/usr/lib/firefox-esr/omni.ja:greprefs.js",
                "/usr/lib/firefox-esr/omni.ja:defaults/pref/*.js",
                "/usr/lib/firefox-esr/defaults/pref/*.js",
                "/usr/lib/firefox-esr/defaults/pref/unix.js",
                "/usr/lib/firefox-esr/omni.ja:defaults/preferences/*.js"
                "/usr/lib/firefox-esr/defaults/preferences/*.js"
            ]

            append_dirs = [ 'defaults/preferences/*.js' ]
            if os.path.isdir('/usr/lib/firefox-esr/distribution/bundles'):
                bundles = os.listdir('/usr/lib/firefox-esr/distribution/bundles')
                bundles.sort(reverse=True)
                for d in append_dirs:
                    for bundle in bundles:
                        path = os.path.join('/usr/lib/firefox-esr/distribution/bundles', bundle)
...
def add_info(report, ui):
    ...
    ddproc = Popen(['dpkg-divert', '--truename', '/usr/bin/firefox-esr'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
    truename = ddproc.communicate()
    if ddproc.returncode == 0 and truename[0].strip() != '/usr/bin/firefox-esr':
        ddproc = Popen(['dpkg-divert', '--listpackage', '/usr/bin/firefox-esr'], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, universal_newlines=True)
        diverter = ddproc.communicate()
        report['UnreportableReason'] = "/usr/bin/firefox-esr has been diverted by a third party package (%s)" % diverter[0].strip()
        return

    conf_dir = os.path.join(os.environ["HOME"], ".mozilla", "firefox-esr")
    appini = AppINIParser('/usr/lib/firefox-esr')
    ...
    if 'firefox-esr' == 'firefox-trunk':
        report["Channel"] = 'nightly'
        append_tag('nightly-channel')
        if report["SourcePackage"] == 'firefox-trunk':
            report["SourcePackage"] = 'firefox'
    else:
        channelpref = Prefs(None, ['/usr/lib/firefox-esr/defaults/pref/channel-prefs.js'], whitelist = [ r'app\.update\.channel' ])
    ...
    if os.path.isdir(apparmor_disable_dir):
        for f in os.listdir(apparmor_disable_dir):
            if f.startswith("usr.bin.firefox-esr"):
                add_apparmor = False
                break
    if add_apparmor:
...
if __name__ == "__main__":
    ...
    D = {}
    D['Package'] = 'firefox-esr'
    D['SourcePackage'] = 'firefox-esr'
    add_info(D, None)
    for KEY in D.keys():
        print('''-------------------%s: ------------------\n%s''' % (KEY, D[KEY]))
