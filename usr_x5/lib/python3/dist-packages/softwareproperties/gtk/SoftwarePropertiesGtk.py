...
class SoftwarePropertiesGtk(SoftwareProperties, SimpleGtkbuilderApp):
    ...
    def __init__(self, datadir=None, options=None, file=None, parent=None):
        ...
        # Initialise and store the apt cache
        self.init_apt_cache()
    ...
    def init_apt_cache(self):
        ...
        self.depcache = apt_pkg.DepCache(self.apt_cache)
        self.records = apt_pkg.PackageRecords(self.apt_cache)
    ...
    def on_driver_changes_apply(self, button):
        ...
        for pkg in self.driver_changes:
            if pkg.current_ver:
                removals.append(self.get_package_id(self.apt_cache, pkg))
                ...
                if 'nvidia' in pkg.name:
                    has_nvidia = True
                    for dep in self.get_dependencies(self.apt_cache, pkg, 'nvidia'):
                        if dep.current_ver:
                            removals.append(self.get_package_id(self.apt_cache, dep))
            else:
                installs.append(self.get_package_id(self.apt_cache, pkg))
    ...
    def gather_device_data(self, device):
        ...
        for pkg_driver_name in device['drivers']:
            ...
            try:
                ...
                installed = pkg.current_ver
                candidate = self.depcache.get_candidate_ver(pkg)

                if candidate is not None:
                    self.records.lookup(candidate.file_list[0])
                    description = _("Using {} from {}").format(self.records.short_desc, pkg_driver_name)
                else:
                    description = _("Using {}").format(pkg_driver_name)
            except KeyError:
    ...
    def set_driver_action_status(self):
        ...
        for device in self.devices:
            for pkg_name in self.devices[device]['drivers']:
                pkg = self.apt_cache[pkg_name]
                if not self.devices[device]['drivers'][pkg_name]['free'] and pkg.current_ver:
