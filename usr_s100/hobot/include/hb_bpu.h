...
#define BPU_VERSION_MINOR 2
#define BPU_VERSION_PATCH 15
...
typedef enum {
	...
	BPU_SR = 0x0004, /**< Shared */
	BPU_SG = 0x0008, /**< SG Map */
} hb_bpu_map_mode_t;

/**
 * @enum hb_bpu_core_lock_flag_t
 * Describe lock flags for bpu core lock
 */
typedef enum {
	BPU_CORE_LOCK_DEFAULT 					= 0x0000, /**< Default lock, block when other process hold the lock and block other process process task*/
	BPU_CORE_LOCK_NO_BLOCK 					= 0x0001, /**< Do not block when other process hold the lock */
	BPU_CORE_LOCK_ALLOW_OTHER_PROCESS_TASK	= 0x0002, /**< Allow other process without the core lock to submit task */
} hb_bpu_core_lock_flag_t;

typedef struct {uint64_t opaque;} hb_bpu_core_t; /**< the bpu core  */
typedef struct {uint64_t opaque;} hb_bpu_core_lock_t; /**< the bpu core lock */
...
typedef int32_t (*hb_bpu_task_pre_process_cb_t)(hb_bpu_task_t task, uint32_t core_index, void *data);
 ...
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Set the bpu task expected deadline time, Relative time and absolute
 * time have and can only have one of 0.
 *
 * @param[in] task: the valid bpu task handle
 * @param[in] rela_eadline: the relative deadline time(unit:ms)
 * @param[in] abs_deadline: the absolute deadline time(Monotic time, unit:ms)
 *
 * @retval =BPU_OK: success
 * @retval !=BPU_OK: failure, reference hb_bpu_err_t
 * @compatibility HW: Super
 ...
 * @design
 */
hb_bpu_err_t hb_bpu_task_set_deadline(hb_bpu_task_t task, uint64_t rela_deadline, uint64_t abs_deadline);
...
} hb_bpu_map_iova_info_t; /**< the bpu iova infomation, when virt_addr == 0, iova info is invalid */

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Map memory range to bpu core for bpu core dev use, and return bpu map handle, and output address map iova information
 *
 * @param[out] map: bpu map handle pointer to assign return value
 * @param[in] core: valid bpu core handle
 * @param[in] addr: to map memory range base, should align PAGE(0x1000) size
 * @param[in] size: to map memory range size
 * #param[out] iova_infos: space to store output iova informations, when iova_info = NULL, no iova info will out
 * #param[in/out] max_info_num: max iova infos number for store when in; real valid iova infos number when out; when max_info_num = NULL, no iova info will out
 *
 * @retval =BPU_OK: success
 * @retval !=BPU_OK: failure, reference hb_bpu_err_t
 * @compatibility HW: Super
 * @compatibility SW: 2.2.8
 *
 * @callgraph
 * @callergraph
 * @design
 */
hb_bpu_err_t hb_bpu_map_iova_info(hb_bpu_map_t *map, hb_bpu_core_t core, uint64_t addr, uint64_t size, hb_bpu_map_iova_info_t *iova_infos, uint32_t *max_info_num);

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Get the bpu mem address's map iova information
 *
 * @param[in] addr: bpu mem address
 * @param[in] core_indexes: bpu core index array, address of core index in array while be output
 * #param[in] core_num: core number of core index array
 * #param[out] iova_infos: space to store output iova informations, when iova_info = NULL, no iova info will out
 * #param[in/out] max_info_num: max iova infos number for store when in; real valid iova infos number when out; when max_info_num = NULL, no iova info will out
 *
 * @retval =BPU_OK: success
 ...
 * @design
 */
hb_bpu_err_t hb_bpu_get_map_iova_info(uint64_t addr, const uint32_t *core_indexes, uint32_t core_num, hb_bpu_map_iova_info_t *iova_infos, uint32_t *max_info_num);
 ...
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Set task pre process callback, callback will be call when
 * user use hb_bpu_core_process to procss task.
 *
 * @param[in] task: the task which to set callback
 * @param[in] task_pre_process_cb: the callback function
 * @param[in] data: data ptr which will passing to callback function
 *
 * @return the real size of feature data
 * @compatibility HW: Super
 * @compatibility SW: 2.1.3
 *
 * @callgraph
 * @callergraph
 * @design
 */
hb_bpu_err_t hb_bpu_task_set_pre_process_cb(hb_bpu_task_t task, hb_bpu_task_pre_process_cb_t task_pre_process_cb, void *data);

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Get the bind core index before task process
 *
 * @param[in] task: the task which to set callback
 * @param[in] core: to bind core from core
 * @param[out] bind_core_index: to out the optimal core index

 * @retval =BPU_OK: success
 * @retval !=BPU_OK: failure, reference hb_bpu_err_t
 * @compatibility HW: Super
 * @compatibility SW: 2.2.5
 *
 * @callgraph
 * @callergraph
 * @design
 */
hb_bpu_err_t hb_bpu_task_bind_core(hb_bpu_task_t task, hb_bpu_core_t core, uint32_t *bind_core_index);


typedef struct {uint64_t opaque;} hb_bpu_bufcnt_t; /**< the bpu bufcnt */

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Allocate bpu core bufcnt
 *
 * @param[in] core_index: to alloc bpu core index
 * @param[out] bufcnt: to out allocate bufcnt
 ...
 * @design
 */
hb_bpu_err_t hb_bpu_core_alloc_bufcnt(uint32_t core_index, hb_bpu_bufcnt_t *bufcnt);

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Free allocated bpu core bufcnt
 *
 * @param[in] bufcnt: the allocated bufcnt
 ...
 * @design
 */
hb_bpu_err_t hb_bpu_core_free_bufcnt(hb_bpu_bufcnt_t bufcnt);

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief To increase or decrease bufcnt value
 *
 * @param[in] bufcnt: the allocated bufcnt
 * @param[in] bias: to set bufcnt bias value
 ...
 * @design
 */
hb_bpu_err_t hb_bpu_core_bufcnt_update(hb_bpu_bufcnt_t bufcnt, int16_t bias);

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Get bufcnt value
 *
 * @param[in] bufcnt: the allocated bufcnt
 * @param[out] value: output the bufcnt value
 ...
 * @design
 */
hb_bpu_err_t hb_bpu_core_bufcnt_get(hb_bpu_bufcnt_t bufcnt, uint16_t *value);

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Get bufcnt core index and index
 *
 * @param[in] bufcnt: the allocated bufcnt
 * @param[out] core_index: the bufcnt core index
 * @param[out] bufcnt_index: bufcnt index
 ...
 * @design
 */
hb_bpu_err_t hb_bpu_core_bufcnt_index(hb_bpu_bufcnt_t bufcnt, uint32_t *core_index, uint32_t *bufcnt_index);

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Lock bpu core for excluse to process task
 *
 * @param[out] lock: bpu core lock
 * @param[in] core_mask: core mask
 * @param[in] flags: lock flags
 *
 * @retval =BPU_OK: success
 * @retval !=BPU_OK: failure, reference hb_bpu_err_t
 *
 ...
 * @design
 */
hb_bpu_err_t hb_bpu_core_lock(hb_bpu_core_lock_t *lock, uint32_t core_mask, uint32_t flags);

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief unlock bpu core lock which locked
 *
 * @param[in] lock: bpu core lock
 *
 * @retval =BPU_OK: success
 * @retval !=BPU_OK: failure, reference hb_bpu_err_t
 *
 ...
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Get the bpu core physics index
 *
 * @param[in] core_index: the user core index
 *
 ...
 * @design
 */
int32_t hb_bpu_core_phy_idx(uint32_t core_index);

typedef struct {
	uint64_t total_ns;
	uint64_t start_ns; /**< Start timestamp(CLOCK_MONOTONIC_RAW) */
	uint64_t end_ns; /**< End timestamp(CLOCK_MONOTONIC_RAW) */
} hb_bpu_task_process_time_t; /**< the bpu task process time */

/**
 * @NO{S04E01C01I}
 * @ASIL{B}
 * @brief Get the bpu task processing time when task done
 *
 * @param[in] task: the valid bpu task handle
 * @param[out] soft_time: between user submit and trigger user done(write to driver->report to user)
 * @param[out] hw_time: between bpu hardware start process and hardware process done
 *
 * @retval =BPU_OK: success
 * @retval !=BPU_OK: failure, reference hb_bpu_err_t
 *
 * @compatibility HW: Super
 * @compatibility SW: 2.2.15
 *
 * @callgraph
 * @callergraph
 * @design
